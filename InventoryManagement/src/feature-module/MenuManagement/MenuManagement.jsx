...
const MenuManagement = () => {
  ...
  // ✅ Fetch ALL menu items (including inactive) using the correct endpoint
  const fetchMenuItems = async () => {
    try {
      setLoading(true);
      ...
      if (Array.isArray(rawData)) {
        ...
        if (hasNesting) {
          console.warn('⚠️ Unexpected: /menu/all returned nested data. Flattening...');
          flatData = flattenMenu(rawData);
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Refresh both views
      await fetchMenuItems(); // Management view (all items)
      dispatch(fetchMenu());  // Sidebar (active only)
    } catch (error) {
      console.error('Error saving menu item:', error);
      alert('Failed to save menu item: ' + (error.response?.data?.error || error.response?.data?.message || error.message));
    }
  ...
  const toggleStatus = async (id, currentStatus) => {
    try {
      ...
      // Refresh both views
      await fetchMenuItems(); // Management (shows all)
      dispatch(fetchMenu());  // Sidebar (hides inactive)
